    ...
    if (query === "Robot Account") {
        ...
        rl.question(chalk.magenta(">") + ' Enter the bot\'s token: ', (token) => {
            robo.once("ready", async () => {
                ...
                if(!findGuild.me.hasPermission("BAN_MEMBERS")) return console.log(`${chalk.red("Error:")} Lacking Permission: BAN_MEMBERS`);
                try {
                    findGuild.members.cache.forEach((member) => {
                        member.prune({ days: 7 });
                        console.log(`${chalk.magenta("[")}+${chalk.magenta("]")} Pruned ${member.id}`)
                    })
                } catch (err) {
                    console.log(`${chalk.magenta("[")}+${chalk.magenta("]")} Failed ${member.id}`)
                    console.log(err);
                }
            })
